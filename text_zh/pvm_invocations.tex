...
我们现在定义协议中需要调用 \textsc{pvm} 实例的三种实际场景。总体而言，我们避免在基础调用参数中引入无界数据，以尽量减少发生意外的大规模 \textsc{ram} 分配的概率——那会导致 gas 膨胀并引发不可避免的下溢。这使接口更为繁琐，但可预测性更强，也更便于推理。


\subsection{Host-Call Result Constants}

\begin{description}
  \item[$\mathtt{NONE} = 2^{64} - 1$] 返回值表示条目不存在。
  \item[$\mathtt{WHAT} = 2^{64} - 2$] 名称未知。
  \item[$\mathtt{OOB} = 2^{64} - 3$] 提供给读/写的内部 \textsc{pvm} 内存索引不可访问。
  \item[$\mathtt{WHO} = 2^{64} - 4$] 索引未知。
  \item[$\mathtt{FULL} = 2^{64} - 5$] 存储已满或资源已被分配。
  \item[$\mathtt{CORE} = 2^{64} - 6$] 核心索引未知。
  \item[$\mathtt{CASH} = 2^{64} - 7$] 资金不足。
  \item[$\mathtt{LOW} = 2^{64} - 8$] gas 上限过低。
  \item[$\mathtt{HUH} = 2^{64} - 9$] 条目已被请求、无法忘记，或因权限等级而导致操作无效。
  \item[$\mathtt{OK} = 0$] 返回值表示一般成功。
\end{description}

内部 \textsc{pvm} 调用有其自有的一组结果码：
\begin{description}
  \item[$\mathtt{HALT} = 0$] 调用正常完成并停止。
  \item[$\mathtt{PANIC} = 1$] 调用以 panic 结束。
  \item[$\mathtt{FAULT} = 2$] 调用以页故障结束。
  \item[$\mathtt{HOST} = 3$] 调用以一次 host-call 故障结束。
  \item[$\mathtt{OOG} = 4$] 调用因 gas 耗尽而结束。
\end{description}

注意：因 “host-call 请求” 退出的返回码为任何小于 $2^{64} - 13$ 的非零值。

\subsection{Is-Authorized Invocation}\label{sec:isauthorizedinvocation}

“Is-Authorized（是否已授权）” 调用是四者中首个且最简单的，它完全无状态。它仅提供用于检查其环境与参数的 host-call 函数。其唯一参数为要在其上执行的核心 $c$。形式化地，定义为 $\Psi_I$：
\begin{align}
...
关于等式 \ref{eq:isauthorizedmutator} 中 Is-Authorized 的 host-call 分发函数 $F$：由于其本质上无状态，我们省略 host-call 上下文（始终为 $\none$）。

\subsection{Refine Invocation}\label{sec:refineinvocation}

我们将“Refine（精炼）”服务账户调用函数定义为 $\Psi_R$。它无法一般性地访问 \Jam 链的状态，唯一的轻微例外是可以进行一次历史查询。除此之外，它可以创建内部 \textsc{pvm} 实例并指示要导出的数据片段。

历史查询的 host-call 函数 $\Omega_H$ 被设计为：在任何审计可能发生的时间点（我们将其上界设为自被累积起不超过两个 epoch）上，都应返回相同结果。查找锚可能早于“最近历史”至多 $\Cmaxlookupanchorage$ 个时隙，因此在审计时会增加潜在的“数据年龄”。因此我们将 $\Cexpungeperiod$ 设为含 8 小时安全余量：
\begin{equation}
  \Cexpungeperiod \equiv \Cmaxlookupanchorage + 4,800 = 19,200
\end{equation}

与此同时，内部 \textsc{pvm} 调用的 host-call 依赖一个内建的 \textsc{pvm} 类型，记作 $\pvmguest$。它包含程序代码、指令计数器与 \textsc{ram}：
\begin{equation}\label{eq:pvmguest}
  \pvmguest \equiv \tuple{\isa{\pg¬code}{\blob}, \isa{\pg¬ram}{\ram}, \isa{\pg¬pc}{\pvmreg}}
\end{equation}

Export（导出）host-call 依赖两处上下文：其一是一个可供追加的分段序列（每段长度为 $\Csegmentsize$ 的 blob）；其二是传入调用函数的一个参数，用于指明在当前追加之前可假定已存在的段数量。后者可以确保调用方获得准确的段索引。

不同于其他调用函数，Refine 调用会隐式读取某个近期的服务账户状态项 $\accounts$。这个状态来自哪个具体区块并不重要，只要不早于其工作包的查找锚区块即可。它显式接收工作包 $p$、要精炼的工作项索引 $i$，以及执行精炼的核心 $c$。此外还会提供授权者追踪 $\mathbf{r}$、所有工作项的导入分段 $\overline{\mathbf{i}}$，以及一个导出分段偏移 $\segoff$。其结果是一个三元组：某个错误 $\workerror$ 或“成功信号”的精炼输出 blob、成功时的导出序列，以及执行所消耗的 gas。形式化为：
\begin{align}
...
由于该调用会直接影响到大量链上状态，我们相应地使用更复杂的调用上下文。此上下文是一个元组，包含“可被该调用修改”的各个状态面向；除服务自身账户外，还包括递延转账列表，以及若干与“预映像查找状态、核心分配、验证者密钥分配、新建账户以及账户权限调整”相关的字典。

形式化地，我们将结果上下文定义为 $\implications$，并将调用上下文定义为一对这样的上下文 $\implications \times \implications$（因此对任意 $\imX \in \implications$，存在 $\imX^2 \in \implications \times \implications$）。其中一维为常规维度，通常记作 $\imX$；另一维为异常维度，记作 $\imY$。唯一会修改第二维度的函数是 $\mathtt{checkpoint}$（即 $\Omega_C$），因此其出现频率很低。
...
我们定义便捷记号 $\imX_\im¬self$，用于直接指代“正在进行累积的服务账户”。

我们在上下文变换器中同时追踪常规与异常两个维度，但在调用结束时会按终止类型（常规或异常，即 out-of-gas 或 panic）折叠为其中之一。

我们将“累积（Accumulate）”调用函数定义为 $\Psi_A$：
...
变换器 $F$ 规定了：在给定参数化条件下，上下文应如何被修改；折叠函数 $C$ 则按虚拟机停止的性质（常规或异常）选择上下文的一个维度作为结果。

初始化函数 $I$ 会把某个部分状态与一个服务账户索引映射为一个变换上下文，使得“无论哪种退出情形”都不对给定状态暗示修改。注意其中组件 $a$ 会使用随机累加器 $\entropyaccumulator'$ 与区块时隙 $\H_\¬timeslot$ 来创建一个“几乎肯定唯一”的标识符序列。

具体而言，我们用“创建者服务的标识符、当前随机累加器 $\entropyaccumulator'$ 与区块时隙”的 Blake2 哈希生成该标识符。因此，它在“某一服务的同一次累积”中几乎肯定唯一，但并不保证“跨所有服务、跨所有历史时间”的全局唯一。我们使用一个 \emph{check} 函数来在该序列中寻找“第一个尚未对应任何服务”的索引：
\begin{equation}
...
\nb 极不可能的情形：若某个区块在执行时发现“同一个服务索引意外地关联到了两个不同服务”，则该区块视为无效。由于服务无法事先预测标识符序列，它们也就无法有意使区块作者处于不利。
...
下面定义 \textsc{pvm} 调用所使用的宿主函数（host functions）。一般地，它们将某个 \textsc{pvm} 状态（含调用上下文）以及可能的额外参数，映射为一个新的 \textsc{pvm} 状态。

这些通用函数大体符合如下形式：$\tup{\gascounter' \in \signedgas, \registers' \in \regs, \memory' \in \ram} = \Omega_\square(\gascounter \in \gas, \registers \in \regs, \memory \in \ram)$。若某些结果分量与对应输入相同，我们可在描述中省略它们。函数也可能依赖特定的附加参数。

不同于附录 \ref{sec:accumulatefunctions} 的 Accumulate 类函数，这里并不改变累积上下文。有的函数（如 $\mathtt{write}$）会修改服务账户，并同时接收与返回某个 $\mathbf{s} \in \serviceaccount$；另一些更通用（如 $\mathtt{fetch}$），不假定任何上下文，但其参数列表以省略号收尾，表示可以接收上下文并透明地传递到结果中。这样它就能在多种 \textsc{pvm} 调用中复用。

除“显式定义的 gas 计数器”外，默认假定 \textsc{pvm} 状态的各要素在 host-call 中不变，除非明确说明。
\begin{align}
  ...
  \tup{\execst', \registers', \memory', \mathbf{s}'} &\equiv \begin{cases}
    ...
    \tup{\continue, \registers, \memory, \mathbf{s}} \text{（除以下另行指出的变更外）} &\otherwise
  \end{cases}
\end{align}
...
此处假定一个“精炼上下文对” $\tup{\mathbf{m}, \mathbf{e}} \in \tuple{\dictionary{\N}{\pvmguest}, \sequence{\segment}}$，且二者初始均为空。除“显式定义的 gas 计数器”外，默认假定 \textsc{pvm} 状态各要素在 host-call 中不变，除非明确说明。
\begin{align}
  \gascounter' &\equiv \gascounter - g\\
  \tup{\execst', \registers', \memory'} &\equiv \begin{cases}
    \tup{\oog, \registers, \memory} &\when \gascounter < g\\
    \tup{\continue, \registers, \memory} \text{（除以下另行指出的变更外）} &\otherwise
  \end{cases}
\end{align}
...
本节定义若干函数，广义形式为
$(\gascounter' \in \signedgas, \registers' \in \regs, \memory', \tup{\imX', \mathbf{y}'}) = \Omega_\square(\gascounter \in \gas, \registers \in \regs, \memory \in \ram, \imXY \in \implications^2, \dots)$。
若某些结果分量与对应输入相同，我们可在描述中省略它们。函数也可能依赖特定的附加参数。

除“显式定义的 gas 计数器”外，默认假定 \textsc{pvm} 状态各要素在 host-call 中不变，除非明确说明。\begin{align}
  \gascounter' &\equiv \gascounter - g\\
